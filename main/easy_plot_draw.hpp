...
namespace easy_plot {
    namespace tools {
        ...
        private:
            ...
            /** \brief Получить ненормализованное значение Y
             * \param ny нормализованное значение
             * \return неНормализованное значение Y
             */

            inline double get_Y(double y) {
                if (max_y == min_y) return 0.0;
                return ((y + 1.0) / 2 * (max_y - min_y)) + min_y;
            }

            /** \brief Получить ненормализованное значение X
             * \param ny нормализованное значение
             * \return неНормализованное значение X
             */

            inline double get_X(double x) {
                if (max_x == min_x) return 0.0;
                return ((x + 1.0) / 2 * (max_x - min_x)) + min_x;
            }
        ...
        public:
            ...
            bool is_axis_signature = true; /* Флаг который подписывает значения осей*/
            ...
            void init(const std::string& name, const WindowSpec& wstyle, const std::vector<std::vector<T1>>& x, const std::vector<std::vector<T2>>& y, const std::vector<LineSpec>& styles) {
                ...
                window_style = wstyle;
                /*if (!wstyle.is_grid) {
                    window_style.grid_period = 1.0 / (x[0].size()/4);
                }*/
                line_style = styles;
            ...
            template <typename T1>
            Drawing(const std::string& name, const WindowSpec& wstyle, const std::vector<std::vector<T1>>& x, const std::vector<std::vector<T1>>& y, const std::vector<LineSpec>& styles) {
                init(name, wstyle, x, y, styles);
            };
            ...
            void draw() {
                ...
                // подписываем значения осей
                if (is_axis_signature) {
                    if (window_style.grid_period != 0.0) {
                        //render_spaced_bitmap_string(0 - glutBitmapLength(window_style.font, (const unsigned char*)'0'), 0- glutBitmapHeight(window_style.font), 0.0, window_style.font,(std::string) "0");
                        for (double y = -1.0; y < 1.0; y += window_style.grid_period) {
                            glColor3f(window_style.tr, window_style.tg, window_style.tb);
                            std::string text_y(std::to_string(std::round(get_Y(y) * 100000) / 100000));
                            text_y.erase(text_y.find('.') + 7);
                            render_spaced_bitmap_string(-1.0 - convert_pixel_to_relative_len(glutBitmapLength(window_style.font, (const unsigned char*)text_y.c_str()), width), y, 0.0, window_style.font, text_y);
                        }
                        for (double x = -1.0; x < 1.0; x += 1.0 + get_x(1)) {
                            glColor3f(window_style.tr, window_style.tg, window_style.tb);
                            std::string text_x(std::to_string(std::round(get_X(x) * 100) / 100));
                            text_x.erase(text_x.find('.') + 3);
                            render_spaced_bitmap_string(x, -1.0 - convert_pixel_to_relative_len(glutBitmapHeight(window_style.font), height), 0.0, window_style.font, text_x);
                        }
                    }
                }
